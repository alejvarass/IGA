...
Begin VB.Form FrmAnalisis 
   ...
   Begin TabDlg.SSTab SSTab3 
      ...
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      TabCaption(0)   =   "Analisis Cromatogr�fico"
      ...
      Begin VB.Frame Frame2 
         ...
         Top             =   3600
         Width           =   7665
         Begin VB.CheckBox ChkMostrarTodosLosAn�lisisTemporal 
            BackColor       =   &H00E8E8E8&
            Caption         =   "Mostrar todos los an�lisis"
            Height          =   210
            Left            =   45
            TabIndex        =   4
            Top             =   3225
   ...
   Begin TabDlg.SSTab SSTab4 
      ...
      Begin iPlotLibrary.iPlotX iPlotMasterLog 
         ...
         YAxis1.InnerMargin=   5
         YAxis1.OuterMargin=   5
         YAxis1.Title    =   "Gas - Cromatograf�a"
         YAxis1.TitleMargin=   0.25
         YAxis1.TitleFont.Charset=   1
         YAxis1.TitleFont.Color=   16777215
         YAxis1.TitleFont.Height=   -13
...
' Layout autoscaling helpers for child form (mejoradas)
Private m_bLayoutSaved_Analisis As Boolean
Private m_origWidth_Analisis As Long
Private m_origHeight_Analisis As Long
...
' Save original layout for this child form, incluyendo controles dentro de SSTab
Public Sub SaveLayout()
    Dim ctrl As Control
    Dim rd As Long, bd As Long
    Dim aH As String, aV As String
    Dim mask As String
    Dim sst As Control
    Dim t As Long, i As Long
    Dim tCtrl As Control
    If m_bLayoutSaved_Analisis Then Exit Sub
    On Error Resume Next
    m_origWidth_Analisis = Me.ScaleWidth
    m_origHeight_Analisis = Me.ScaleHeight

    ' Guardar controles directos del form (igual que antes)
    For Each ctrl In Me.Controls
        On Error Resume Next
        rd = m_origWidth_Analisis - (ctrl.Left + ctrl.Width)
        bd = m_origHeight_Analisis - (ctrl.Top + ctrl.Height)
        If m_origWidth_Analisis > 0 Then
            If (ctrl.Width / m_origWidth_Analisis) >= 0.35 Then
                aH = "LR"
            Else
                aH = "L"
            End If
        Else
            aH = "L"
        End If
        If m_origHeight_Analisis > 0 Then
            If (ctrl.Height / m_origHeight_Analisis) >= 0.35 Then
                aV = "TB"
            Else
                aV = "T"
            End If
        ...
        ' Forzamos SSTab a estirar ambos ejes (mejor detectar la clase)
        If InStr(LCase(TypeName(ctrl)), "sstab") > 0 Or InStr(LCase(TypeName(ctrl)), "tab") > 0 Then
            mask = "LRTB"
        Else
            mask = aH & aV
        End If

        ctrl.Tag = "ORIG|" & ctrl.Left & "|" & ctrl.Top & "|" & ctrl.Width & "|" & ctrl.Height & "|" & rd & "|" & bd & "|A=" & mask

        ' Si es un SSTab, adem�s guardamos los controles que contiene en cada Tab
        If InStr(LCase(TypeName(ctrl)), "sstab") > 0 Or InStr(LCase(TypeName(ctrl)), "tab") > 0 Then
            Set sst = ctrl
            On Error Resume Next
            ' TabCount / Tabs property: intentamos iterar por �ndice
            For t = 0 To sst.Tabs - 1
                ' ControlCount para cada pesta�a
                For i = 0 To sst.Tab(t).ControlCount - 1
                    On Error Resume Next
                    Set tCtrl = sst.Tab(t).Control(i)
                    If Not tCtrl Is Nothing Then
                        ' Guardamos posicion ABSOLUTA respecto al Form: parent.Left + child.Left
                        rd = m_origWidth_Analisis - ((sst.Left + tCtrl.Left) + tCtrl.Width)
                        bd = m_origHeight_Analisis - ((sst.Top + tCtrl.Top) + tCtrl.Height)
                        ' Tag: ORIGTAB|ParentName|TabIndex|AbsLeft|AbsTop|Width|Height|RightDist|BottomDist|A=mask
                        ' Usamos ParentName para localizarlo despu�s
                        mask = "L" ' por defecto anclado a la izquierda; ApplyAnchors tratar� estiramientos dependiendo del ancho/alto original
                        tCtrl.Tag = "ORIGTAB|" & sst.Name & "|" & CStr(t) & "|" & (sst.Left + tCtrl.Left) & "|" & (sst.Top + tCtrl.Top) & "|" & tCtrl.Width & "|" & tCtrl.Height & "|" & rd & "|" & bd & "|A=" & mask
                    End If
                Next i
            Next t
        End If
    Next
    m_bLayoutSaved_Analisis = True
End Sub

' Aplica anclajes (incluye controles dentro de SSTab)
Public Sub ApplyAnchors()
    Dim ctrl As Control
    Dim arr As Variant
    Dim oL As Long, oT As Long, oW As Long, oH As Long
    Dim rd As Long, bd As Long
    Dim mask As String
    Dim newL As Long, newT As Long, newW As Long, newH As Long
    Dim sst As Control
    Dim t As Long, i As Long
    Dim tCtrl As Control
    On Error Resume Next

    ' Primero redimensionar/colocar los controles directos del formulario (incluyendo el SSTab en s�)
    For Each ctrl In Me.Controls
        If Len(ctrl.Tag) > 5 Then
            If Left$(ctrl.Tag, 5) = "ORIG|" Then
                arr = Split(ctrl.Tag, "|")
                If UBound(arr) >= 7 Then
                    ...
                    bd = CLng(arr(6))
                    mask = arr(7)
                    If Left$(mask, 2) = "A=" Then mask = Mid$(mask, 3)
                    newL = oL: newT = oT: newW = oW: newH = oH
                    ' Horizontal
                    If InStr(mask, "L") > 0 And InStr(mask, "R") > 0 Then
                        newL = oL
                        newW = Me.ScaleWidth - oL - rd
                        If newW < 1 Then newW = 1
                    ElseIf InStr(mask, "R") > 0 Then
                        newL = Me.ScaleWidth - rd - oW
                    Else
                        newL = oL
                    End If
                    ' Vertical
                    If InStr(mask, "T") > 0 And InStr(mask, "B") > 0 Then
                        newT = oT
                        newH = Me.ScaleHeight - oT - bd
                        If newH < 1 Then newH = 1
                    ElseIf InStr(mask, "B") > 0 Then
                        newT = Me.ScaleHeight - bd - oH
                    Else
                        newT = oT
                    End If
                    On Error Resume Next
                    ctrl.Move newL, newT, newW, newH
                    On Error GoTo 0
                End If
            End If
        End If
    ...
    ' Ahora ajustar los controles que hab�amos guardado como ORIGTAB (controles dentro de SSTab)
    For Each ctrl In Me.Controls
        If InStr(LCase(TypeName(ctrl)), "sstab") > 0 Or InStr(LCase(TypeName(ctrl)), "tab") > 0 Then
            Set sst = ctrl
            On Error Resume Next
            For t = 0 To sst.Tabs - 1
                For i = 0 To sst.Tab(t).ControlCount - 1
                    ...
                    Set tCtrl = sst.Tab(t).Control(i)
                    If Not tCtrl Is Nothing Then
                        If Len(tCtrl.Tag) > 7 And Left$(tCtrl.Tag, 7) = "ORIGTAB" Then
                            arr = Split(tCtrl.Tag, "|")
                            ' ORIGTAB|ParentName|TabIndex|AbsLeft|AbsTop|Width|Height|rd|bd|A=mask
                            If UBound(arr) >= 9 Then
                                oL = CLng(arr(4)) ' absolute original left
                                oT = CLng(arr(5)) ' absolute original top
                                oW = CLng(arr(6))
                                oH = CLng(arr(7))
                                rd = CLng(arr(8))
                                bd = CLng(arr(9))
                                mask = arr(10)
                                If Left$(mask, 2) = "A=" Then mask = Mid$(mask, 3)
                                ' Volver a calcular posici�n absoluta nueva seg�n el tama�o del form:
                                ' Por simplicidad: mantendremos la distancia izquierda proporcional al ancho,
                                ' y distancia superior proporcional al alto, salvo que haya que pegar al borde.
                                Dim sx As Double, sy As Double
                                If m_origWidth_Analisis > 0 Then sx = Me.ScaleWidth / m_origWidth_Analisis Else sx = 1
                                If m_origHeight_Analisis > 0 Then sy = Me.ScaleHeight / m_origHeight_Analisis Else sy = 1
                                newL = CLng((oL) * sx)
                                newT = CLng((oT) * sy)
                                newW = CLng(Max(1, oW * sx))
                                newH = CLng(Max(1, oH * sy))
                                ' Convertir absoluta a relativa dentro del SSTab: leftRel = newAbsLeft - sst.Left
                                Dim leftRel As Long, topRel As Long
                                leftRel = newL - sst.Left
                                topRel = newT - sst.Top
                                If leftRel < 0 Then leftRel = 0
                                If topRel < 0 Then topRel = 0
                                On Error Resume Next
                                tCtrl.Move leftRel, topRel, newW, newH
                                On Error GoTo 0
                            End If
                        End If
                    End If
...
' Scale child controls according to current size (incluye SSTab internals)
Public Sub ScaleLayout()
    Dim ctrl As Control
    Dim arr As Variant
    Dim oL As Long, oT As Long, oW As Long, oH As Long
    Dim sx As Double, sy As Double
    ...
    If Not m_bLayoutSaved_Analisis Then SaveLayout
    On Error Resume Next
    If m_origWidth_Analisis = 0 Or m_origHeight_Analisis = 0 Then Exit Sub
    sx = Me.ScaleWidth / m_origWidth_Analisis
    sy = Me.ScaleHeight / m_origHeight_Analisis

    ' Escalar controles directos del form
    For Each ctrl In Me.Controls
        If Len(ctrl.Tag) > 5 Then
            If Left$(ctrl.Tag, 5) = "ORIG|" Then
                arr = Split(ctrl.Tag, "|")
    ...
    ' Escalar controles dentro de SSTab (guardados como ORIGTAB con coordenadas absolutas)
    For Each ctrl In Me.Controls
        If InStr(LCase(TypeName(ctrl)), "sstab") > 0 Or InStr(LCase(TypeName(ctrl)), "tab") > 0 Then
            Set sst = ctrl
            On Error Resume Next
                ...
                For i = 0 To sst.Tab(t).ControlCount - 1
                    ...
                    If Not tCtrl Is Nothing Then
                        If Len(tCtrl.Tag) > 7 And Left$(tCtrl.Tag, 7) = "ORIGTAB" Then
                            arr = Split(tCtrl.Tag, "|")
                            If UBound(arr) >= 9 Then
                                ' arr(4)=AbsLeft arr(5)=AbsTop arr(6)=Width arr(7)=Height
                                oL = CLng(arr(4))
                                oT = CLng(arr(5))
                                oW = CLng(arr(6))
                                oH = CLng(arr(7))
...
Private Sub CmdConsultar_Click()
    ...
    If Txtfecha.Value <> 0 Then
        ...
        If Txthora.Text <> "" Then
            ...
            iPlotGases.XAxis(0).Span = ObtenerSpanEnD�as(FechaInicio, FechaFin)
...
Error:
    ...
    If Err.Number <> 0 Then
        ...
        frmMsg.MostrarMsg "M�dulo: CmdGuardar_Click." & Chr(10) & Err.Number & " - " & Err.Description, "Error", MdiPrincipal
...
Private Sub ChkMostrarTodosLosAn�lisisTemporal_Click()
    ...
    If Not gObjPozoActivo Is Nothing Then
        ...
        If ChkMostrarTodosLosAn�lisisTemporal.Value <> 1 Then
...
Error:
    ...
    If Err.Number <> 0 Then
        ...
        frmMsg.MostrarMsg "M�dulo: ChkMostrarTodosLosAn�lisisTemporal." & Chr(10) & Err.Number & " - " & Err.Description, "Error", MdiPrincipal
...
Public Sub LimpiarYConsultarDatosTiempo()
    ...
    If Not gObjPozoActivo Is Nothing Then
        ...
        If ChkTiempoReal = 1 Then
            ...
            iPlotGases.XAxis(0).Span = ObtenerSpanEnD�as(FechaInicio, DateAdd("n", gZonaMuerta, FechaFin))
            iPlotGases.XAxis(0).Min = Month(FechaInicio) * 30 + Day(FechaInicio) + Hour(FechaInicio) / 24 + Minute(FechaInicio) / (24 * 60) + Second(FechaInicio) / (24# * 60# * 60#)
            'here
            objPozoGasTiempos.dbPozoGasesTiempo gObjPozoActivo.IdPozo, FechaInicio, FechaFin
...
Error:
If Err.Number <> 0 Then
    frmMsg.MostrarMsg "No se encuentra el archivo de propiedades dle gr�fico", "Error de archivo"
    Err.Clear
    End If
...
Error:
    ...
    If Err.Number <> 0 Then
        ...
        frmMsg.MostrarMsg "M�dulo: BuscarComponentes." & Chr(10) & Err.Number & " - " & Err.Description, "Error", MdiPrincipal
        Err.Clear
    End If
        ...
        If NewString <> "" Then
            ...
            If IsNumeric(NewString) Then
                ...
                If Err.Number = 6 Then
                    ...
                    frmMsg.MostrarMsg "El n�mero debe ser mayor que 0 (Cero) y menor que 32.767.", "Error", MdiPrincipal
                    Cancel = True
                    Err.Clear
                ...
                ElseIf Numero < 0 Then
                    frmMsg.MostrarMsg "El n�mero debe ser mayor o igual que cero.", "Error", MdiPrincipal
                    Cancel = True
                Else
                    ...
                    If objPozoAnalisComponente Is Nothing Then
                        ...
                        If objPozoAnalis.PozoAnalisComponentes.dbModificarComponente(objPozoAnalisComponente, NumeroComponenteAnterior) Then
                            ...
                            Else
                                gDatos.RollBackTrans
                                Cancel = True
                                frmMsg.MostrarMsg "No se pudo actualizar el gas total cromatogr�fico", "Error", MdiPrincipal
                            End If
                        Else
                            Cancel = True
                            gDatos.RollBackTrans
                        ...
                        End If
                    Else
                        frmMsg.MostrarMsg "Ya existe el componente " & Numero & " en el an�lisis", "Error", MdiPrincipal
                        Cancel = True
                    End If
                End If
            Else
                Cancel = True
                frmMsg.MostrarMsg "Debe ingresar un valor num�rico.", "Error", MdiPrincipal
            End If
        Else
            Cancel = True
        End If
...
Error:
    If Err.Number <> 0 Then
        Cancel = True
        frmMsg.MostrarMsg "M�dulo: lvwArticulos_AfterLabelEdit." & Chr(10) & Err.Number & " - " & Err.Description, "Error", MdiPrincipal
        Err.Clear
    End If
End Sub
...
Public Sub AnalisisTemporalBorrar()
    ...
    If LvwAnalisisTemporal.SelectedItem.SmallIcon = 1 Then
        Respuesta = MsgBox("�Est� seguro que desea borrar el analisis seleccionado?", vbYesNo + vbQuestion)
        Accion = "MARCAR"
    ElseIf LvwAnalisisTemporal.SelectedItem.SmallIcon = 2 Then
        Respuesta = MsgBox("El an�lisis seleccionado es un an�lisis de calibraci�n y se eliminar� de forma permanente. �Est� seguro que desea borrar el analisis seleccionado?", vbYesNo + vbQuestion)
        Accion = "ELIMINAR"
    ElseIf LvwAnalisisTemporal.SelectedItem.SmallIcon = 3 Then
        Respuesta = MsgBox("El an�lisis seleccionado es un an�lisis de gas de yacimiento y se eliminar� de forma permanente. �Est� seguro que desea borrar el analisis seleccionado?", vbYesNo + vbQuestion)
        Accion = "ELIMINAR"
    ElseIf LvwAnalisisTemporal.SelectedItem.SmallIcon = 4 Then
        Respuesta = MsgBox("�Est� seguro que desea restaurar el analisis seleccionado?", vbYesNo + vbQuestion)
        Accion = "DESMARCAR"
    ElseIf LvwAnalisisTemporal.SelectedItem.SmallIcon = 3 Then
        Respuesta = MsgBox("El an�lisis seleccionado es un an�lisis de circulada y se eliminar� de forma permanente. �Est� seguro que desea borrar el analisis seleccionado?", vbYesNo + vbQuestion)
        Accion = "ELIMINAR"
    End If
        ...
        If Not objPozoAnalis Is Nothing Then
            Select Case Accion
                ...
                Case Is = "MARCAR"
                    ...
                    If gObjPozoActivo.PozoAnalisis.dbModificar(objPozoAnalis) Then
                        gDatos.CommitTrans
                        If ChkMostrarTodosLosAn�lisisTemporal.Value = 0 Then
...
Error:
    ...
    If Err.Number <> 0 Then
        ...
        frmMsg.MostrarMsg "M�dulo: AnalisisTemporalBorrarList." & Chr(10) & Err.Number & " - " & Err.Description, "Error", MdiPrincipal
...
Public Sub AnalisisTemporalSeleccionar()
    ...
    If LvwAnalisisTemporal.SelectedItem.SmallIcon = 1 Then
        ...
        If Profundidad <> 0 Then
            ...
            End If
        Else
            frmMsg.MostrarMsg "Debe ingresar la profundidad antes de seleccionar el an�lisis", "Error", MdiPrincipal
            SSTab1.Tab = 0
            TxtProfundidadTemporal.SetFocus
        End If
    Else
        frmMsg.MostrarMsg "Solamente se pueden seleccionar an�lisis normales", "Error", MdiPrincipal
    End If
...
Error:
    ...
    If Err.Number <> 0 Then
        ...
        frmMsg.MostrarMsg "M�dulo: AnalisisTemporalSeleccionar." & Chr(10) & Err.Number & " - " & Err.Description, "Error", MdiPrincipal
...
Error:
    ...
    If Err.Number <> 0 Then
        ...
        frmMsg.MostrarMsg "M�dulo: BuscarAnalisis." & Chr(10) & Err.Number & " - " & Err.Description, "Error", MdiPrincipal
...
Public Sub ComponenteTemporalBorrar()
    ...
    Respuesta = MsgBox("Est� seguro que desea borrar el Componente seleccionado", vbYesNo + vbQuestion)
                ...
                If Not objPozoAnalisComponente Is Nothing Then
                    ...
                    If objPozoAnalis.PozoAnalisComponentes.dbEliminar(objPozoAnalisComponente) Then
                        ...
                        If ComponenteTemporalBorrarList Then
                            ...
                            Else
                                gDatos.RollBackTrans
                                frmMsg.MostrarMsg "No se pudieron actualizar las relaciones cromatogr�ficas", "Error", MdiPrincipal
                            End If
                        Else
                            gDatos.RollBackTrans
                            Unload Me
...
Error:
    ...
    If Err.Number <> 0 Then
        ...
        frmMsg.MostrarMsg "M�dulo: ComponenteTemporalBorrarList." & Chr(10) & Err.Number & " - " & Err.Description, "Error", MdiPrincipal
...
Error:
    ...
    If Err.Number <> 0 Then
        ...
        frmMsg.MostrarMsg "M�dulo: BuscarComponentesTemporales." & Chr(10) & Err.Number & " - " & Err.Description, "Error", MdiPrincipal
...
Error:
    ...
    If Err.Number <> 0 Then
        ...
        frmMsg.MostrarMsg "M�dulo: BuscarComponentesDefinitivos." & Chr(10) & Err.Number & " - " & Err.Description, "Error", MdiPrincipal
        Err.Clear
    End If
...
Error:
    ...
    If Err.Number <> 0 Then
        ...
        frmMsg.MostrarMsg "M�dulo: BuscarRelacionesCromatograficasTemporal." & Chr(10) & Err.Number & " - " & Err.Description, "Error", MdiPrincipal
        Err.Clear
    End If
...
Error:
    ...
    If Err.Number <> 0 Then
        ...
        frmMsg.MostrarMsg "M�dulo: BuscarRelacionesCromatograficasDefinitivo." & Chr(10) & Err.Number & " - " & Err.Description, "Error", MdiPrincipal
        Err.Clear
    End If
...
Error:
    If Err.Number <> 0 Then
        frmMsg.MostrarMsg "M�dulo: ConsultarTodos los cronos ." & Chr(10) & "Ocurri� el error: " & Err.Number & " - " & Err.Description, "Error", MdiPrincipal
        Err.Clear
    End If
    Me.MousePointer = 0
...
Error:
    If Err.Number <> 0 Then
        ...
        frmMsg.MostrarMsg "M�dulo: ConsultarPozoEscaneos ." & Chr(10) & "Ocurri� el error: " & Err.Number & " - " & Err.Description, "Error", MdiPrincipal
        Err.Clear
    End If
End Function
...
Error:
    If Err.Number <> 0 Then
        frmMsg.MostrarMsg "M�dulo: ActualizarGasesEscaneos." & Chr(10) & "Ocurri� el error: " & Err.Number & " - " & Err.Description, "Error", MdiPrincipal
        Err.Clear
    End If
End Sub
